...
/*
   Pasted the names into the source file as an array, use qsort(3), add them.
   Runtime is 0.002s Yawn.
*/
char *names[] = {
	...
	"HAI", "ELDEN", "DORSEY", "DARELL", "BRODERICK", "ALONSO", NULL
};

int strsort(const void *va, const void *vb)
{
	char *a = *((char **)va);
	char *b = *((char **)vb);
	return strcmp(a, b);
...
int main(int argc, char **argv[])
{
	...
	qsort(names, name_count, sizeof(char *), strsort);

	for(i = 0; names[i]; i++) {
		name_total = 0;
		for(j = 0; names[i][j]; j++) {
			name_total += names[i][j] - sub;
		}
