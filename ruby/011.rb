...
# This problem is so tedious, but it upset me a little that I hadn't yet
# solved it.  I used a stupid, sloppy algorithm, and wrote bad code, because I
# figured something like this would be the result:
#   real    0m0.024s
#   user    0m0.023s
#   sys     0m0.000s
# ...and it was.  Those are Ruby 1.9 numbers.  To satisfy your curiousity, if
# you have any, it is 0.084s under 1.8.
...
# Why do them as strings?  Turning them into literal arrays of literal integers
# pushed all of the columns to 81 characters long, before I even added
# indentation.  Boooooo!

def a; $a; end

def extract_prod x, y, xdir, ydir
	(0..3).map { |i|
		xi = x + i * xdir
		yi = y + i * ydir
		return nil unless((a[yi][xi] rescue nil))
		a[yi][xi]
	}.inject(&:*)
end

dirs = (-1..1).map { |dx| (-1..1).map { |dy| [dx, dy] } }.inject(&:+)
dirs.delete([0, 0])
max = 0

(0..a.length).each { |y|
	(0..(a.first.length)).each { |x|
		dirs.each { |d|
			max = [extract_prod(x, y, *d), max].compact.max
		}
	}
}
puts max
